...
export async function getListCompletedRounds(data: { skip: number; limit: number; timeConfig: WINGO_TIME_CONFIG }) {
  const token = localStorage.getItem('token')
  const res: any = await request({
    url: '/admin/trx-wingo/get-completed-round',
    options: {
      method: 'post',
      data,
      headers: {
...
export async function getStatisticCurrentRound(timeConfig: WINGO_TIME_CONFIG) {
  const token = localStorage.getItem('token')
  const res: any = await request({
    url: `/admin/trx-wingo/get-statistic-current-round?timeConfig=${timeConfig}`,
    options: {
      method: 'get',
      headers: {
        Authorization: `Bearer ${token}`
