...
import { DatePicker } from '../../pro-component';
...
const FormSearch = <T, U = any>({
  ...
  const searchConfig = getDefaultSearch(
    propsSearch,
    intl,
    type === 'advancedform'
  );
  ...
  const getErrorInfo = (
    ...
    const errorCount = errors.filter(
      (item: { name: string[]; errors: React.ReactNode[] }) =>
    ...
    const errorList = errors.map(
      (err: { name: string[]; errors: React.ReactNode[] }) => {
