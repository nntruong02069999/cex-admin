...
import {
  FiveDGameCompletedRound,
  FiveDGameStatisticCurrentRound,
  GetListCompletedRoundParams,
} from "@src/interfaces/5DGame";
...
const FiveDGame: React.FC<any> = () => {
  ...
  const [listNextRound, setListNextRound] = useState<FiveDGameCompletedRound[]>(
    []
  );
  ...
  const [statisticCurrentRoundAllInfos, setStatisticCurrentRoundAllInfos] =
    useState<Partial<FiveDGameStatisticCurrentRound>>({});
  ...
  const getListCompletedRounds = async (
    ...
    try {
      const response: any = await fiveDGameServices.getListCompletedRounds(
        params
      );
      if (response?.code === 0) {
  ...
  const getListNextRounds = async () => {
    ...
    try {
      const response: any = await fiveDGameServices.getListNextRounds(
  ...
  const getStatisticCurrentRound = async () => {
    ...
    try {
      const response: any = await fiveDGameServices.getStatisticCurrentRound(
        selectedWingoTimeConfig.value
      );
      if (response?.code === 0) {
  ...
  return (
    <div className="main-game-container">
      ...
      <Button.Group className="main-game-time-config-groups">
        {ARRAY_WINGO_TIME_CONFIGS.slice(0, 4).map((option) => {
      ...
      <section className="main-game-current-game">
        ...
        <Table
          ...
          className="main-game-table game-current-table"
          loading={loadingStatisticCurrentRound}
          rowClassName={(record) =>
