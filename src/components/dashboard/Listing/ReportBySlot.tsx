...
export interface ReportBySlotProps {
  location?: any
}

export interface ReportBySlotState {
  error?: any
  loading?: boolean
  data: any
  fromDate: any
...
export default class ReportBySlot extends Component<
  ...
  ReportBySlotState
> {
  constructor(props: ReportBySlotProps) {
    super(props)
  ...
  handleChangeFactory = async (value: any) => {
    this.setState({ factoryIds: value })
  }
        ...
        <div className="gx-dealclose-header">
          <div>
            <h2 className="h2 gx-mb-2">
              Lượt xem theo buồng sửa (<b>{data?.total}</b> lượt)
            </h2>
          </div>
