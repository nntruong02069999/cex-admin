...
const ListUser = (props) => {
  const { value, onChange, paramsList, agencyId } = props
  ...
  const fetchData = async (params, sort, filter) => {
    ...
    filter = Object.keys(filter).reduce((obj, key) => {
      const newObj = { ...obj }
      if (filter[key] !== null)
        newObj[key] = { contains: getValue(filter[key]) }
      return newObj
    }, {})
    ...
    const response = await userServices.getList(params)
      ...
      <ProTable
        ...
        rowSelection={{
          ...
          renderCell: (checked, record, index, originNode) => {
            return <Tooltip title={'Chọn User'}>{originNode}</Tooltip>
          },
