...
class ListCtrl extends Component<ListCtrlProps, ListCtrlState> {
  ...
  fetchData = async (
    ...
    if (this.props.query.filter) {
      if (typeof this.props.query.filter === "string") {
        Object.assign(filter, JSON.parse(this.props.query.filter));
      } else {
        Object.assign(filter, this.props.query.filter);
      }
    }
    ...
    if (
      rs.data.errorCode === HttpStatusCode.UNAUTHORIZED ||
      rs.data.errorCode === HttpStatusCode.UNAUTHORIZED_TOKEN
    ) {
          ...
          if (!raw) {
            ...
            switch (query.modalType) {
              ...
              case "list":
                ...
                pageModalInfo = await helper.getPage(query.page);
                break;
              case "form":
              default:
  ...
  generateDisplay = (item: any, gridInfo: any) => {
    ...
    switch (type) {
      ...
      case DISPLAY_TYPE.HTML:
        ...
        // // Optional: Add a custom editor for when you want to edit this field
        // item.renderFormItem = (_: any, { type }: any) => {
        //   if (type === "form") {
        //     return null;
        //   }
        ...
        // };
        break;
      case DISPLAY_TYPE.PASSWORD:
        item.render = (value: any) => {
          return <PasswordContent content={value} />;
        };
        break;
  ...
  renderPopup = (modalQuery: any) => {
    const { currentModal: EditModal } = this.state;
    switch (modalQuery.modalType) {
      case "form":
        return (
          <FormCtrl
      ...
      case "list": {
        return <ListCtrl pageInfo={this.state.pageModal} query={modalQuery} />;
      }
      default:
        return (
          <EditModal
  ...
  render() {
    const ViewDetail =
      detailTemplateArray[
        this.state.viewDetail.viewDetailCtrl as keyof typeof detailTemplateArray
      ];
    if (this.state.error)
            ...
            >
              <>
                {this.state.currentModal ? (
                  this.renderPopup(this.state.modalQuery)
                ) : (
                  <Loader />
                )}
              </>
